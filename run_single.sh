...
EXPERIMENT="${EXPERIMENT:-manual-run}"
...
RL="${RL:-1}"
RL_POLICY="${RL_POLICY:-output/rl/policy.npz}"
RL_TICK="${RL_TICK:-1.0}"
...
RL_ENTROPY="${RL_ENTROPY:-1}"

HOP="${HOP:-4}"                                       # broker hop interval (s); also drives source-IP hop
IP_POOL="${IP_POOL:-10.1.0.2,10.1.0.4,10.1.0.5,10.1.0.6,10.1.0.7,10.1.0.8}"  # broker IP pool (comma-separated; single value = IP hop off)
PORT_POOL="${PORT_POOL:-8883,8884,8885,8886,8887,8888,8889}"           # broker port pool (single value = port hop off)
SCMC_POOL="${SCMC_POOL:-10.0.0.2,10.0.0.4,10.0.0.5,10.0.0.6,10.0.0.7,10.0.0.8}" # source-IP pool (single value = source hop off)
PAD_BUCKETS="${PAD_BUCKETS:-128,256,384,512,640,768,1024}"     # payload padding buckets
PAD_INT="${PAD_INT:-3}"                        # padding mutation interval (s)
MBPS="${MBPS:-2}"                              # background replay noise (Mbit/s)
FREQ_POOL="${FREQ_POOL:-0.1,0.2,0.4,0.6,0.8,1}"  # message-frequency pool (single value = freq hop off)
FREQ_INT="${FREQ_INT:-5}"                      # message-frequency mutation interval (s)
# ──────────────────────────────────────────────────────────────────────────────
